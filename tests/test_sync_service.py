...
so no network or subprocess activity occurs.
"""
...
class TestRunSync(unittest.TestCase):
    ...
    def _patches(self, sp_mock, qq_mock, rotated: bool = False):
    ...
    def test_cache_hit_skips_search(self):
        """Second run should pull QQ mapping from cache, no search call."""
        sp_tracks = [_sp_track("sp1", "Song One", "Alice")]
        search_map = {"Song One Alice": [_qq_cand(101, "Song One", "Alice", 200, 0)]}
        sp = _make_sp_mock(sp_tracks)
        ...
        # Second run: QQ playlist now contains the matched song.
        ...
        self.assertEqual(rc, 0)
        # Cache hit — search should NOT have been called.
